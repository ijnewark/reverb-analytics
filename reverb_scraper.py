...
def fetch_sold_listings(
    ...
    with httpx.Client(timeout=10) as client:
        ...
        while page <= max_pages:
            ...
            try:
                r = client.get(url, params=params, headers=config.REVERB_API_HEADERS)
                
                # Retry on rate limit without incrementing page
...
def store_listings(
    ...
    """Store listings in the SQLite database using batch inserts for efficiency."""
    records = []
    ...
    for listing in listings:
        try:
            ...
            print(f"  Warning: Could not process listing {listing.get('id')}: {e}")
...
def flag_deals(
    ...
    # Vectorised miscategorisation check using merge
    # Build expected category lookup
    category_lookup = pd.DataFrame([
        {"canonical_model": model, "expected_category": cat}
        for model, cat in config.MODEL_CATEGORY_MAP.items()
    ])
    
    df = df.merge(category_lookup, on="canonical_model", how="left")
    
    # Vectorised fuzzy match using apply on smaller filtered subset
    # Only check rows where we have both canonical_model and expected_category
    has_expected = df["expected_category"].notna()
    df["miscategorised"] = False
    
    if has_expected.any():
        df.loc[has_expected, "miscategorised"] = df.loc[has_expected].apply(
            lambda row: fuzz.token_set_ratio(row["category_full"], row["expected_category"]) < config.FUZZY_CATEGORY_THRESHOLD,
            axis=1
        )
...
")
    ...
    for _, row in deals.head(20).iterrows():
        ...
        print(f"  Price: {row['price_currency']} {row['price_amount']:.2f} | z-score: {row['z_score']:.2f}")
        print(f"  URL: {row['url']}")
        print()
